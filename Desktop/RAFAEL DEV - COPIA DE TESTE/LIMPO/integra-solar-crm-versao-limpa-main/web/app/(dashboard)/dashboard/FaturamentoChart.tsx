    ...
    >
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-4 text-xs">
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-sm inline-block" style={{ background: '#FFD080' }} />
            <span className="text-white/50">{anoAtual}</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-sm inline-block" style={{ background: 'rgba(255,255,255,0.2)' }} />
      ...
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} barGap={2}>
          ...
          <Bar dataKey="ano_anterior" name={String(anoAnterior)} fill="rgba(255,255,255,0.15)" radius={[4, 4, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  )
